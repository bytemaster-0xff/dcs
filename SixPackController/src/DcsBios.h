...
/*
	...
	namespace DcsBios {
		ProtocolParser parser;
		HardwareSerial *m_dcsSerial;


		void setup(HardwareSerial *dcsSerial) {
			m_dcsSerial = dcsSerial;
			m_dcsSerial->begin(250000);
		}

		void RegisterCommandCallback(void callback(unsigned char cmd, unsigned int address, int value)){
			parser.RegisterCommandCallback(callback);
		}

		void loop() {
			while (Serial1.available()) {
				char ch = m_dcsSerial->read();
		...
		bool tryToSendDcsBiosMessage(const char* msg, const char* arg) {
			m_dcsSerial->write(msg); m_dcsSerial->write(' '); m_dcsSerial->write(arg); m_dcsSerial->write('\n');
			DcsBios::PollingInput::setMessageSentOrQueued();
			return true;
		}
	}
